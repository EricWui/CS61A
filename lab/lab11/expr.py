...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        apply_operands=self.operator.eval(env).apply
        operands=[]
        for operand in self.operands:
            operands.append(operand.eval(env))
        return apply_operands(operands)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        expr=self.body
        args_length=len(self.parameters)
        for index in range(args_length):
            new_env.update({self.parameters[index] : arguments[index]})
