...
def subseqs(s):
    ...
    if not s:
        ...
        subseqs_prev= subseqs(s[1:])
        return subseqs_prev+insert_into_all(s[0],subseqs_prev)
...
def non_decrease_subseqs(s):
    ...
    if not s:
        ...
        subseqs_prev= non_decrease_subseqs(s[1:])
        subseqs_prev_copy=subseqs_prev[:]
        for item in subseqs_prev:
            if item and item[0]<s[0]:
                subseqs_prev_copy.remove(item)
        return subseqs_prev+insert_into_all(s[0],subseqs_prev_copy)
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    for index in range(1,n):
        ans+=num_trees(index)*num_trees(n-index)
    return ans 
...
def merge(incr_a, incr_b):
    ...
    while next_a !=None or next_b !=None:
        if (next_a != None and next_a<next_b) or next_b== None:
            yield next_a
...
def insert(link, value, index):
    ...
    """
    "*** YOUR CODE HERE ***"
    def change_value(link,value):
        if link==Link.empty:
            return
        change_value(link.rest,link.first)
        link.first=value
        
    # Create a new link at the rest of the link
    temp=link
    len=1
    ...
    temp.rest=Link(-1,Link.empty)
    temp=link
    # Move back the rest of link
    while index>0 and temp!=Link.empty:
        temp=temp.rest
        index=index-1
    change_value(temp.rest,temp.first)
    temp.first=value    
...
def deep_len(lnk):
    ...
    if lnk is Link.empty:
        return 0
    elif not isinstance(lnk,Link) or lnk.rest is Link.empty:
        return 1
    else:
