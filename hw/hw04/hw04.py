...
def balanced(m):
    ...
    if is_planet(m):
        return True
    ans=balanced(end(left(m))) and balanced(end(right(m)))
    if ans:
...
def replace_thor_at_leaf(t, thors_replacement):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        return tree(thors_replacement if label(t)=='thor' else label(t),branches(t))
    return tree(label(t),[replace_thor_at_leaf(branch,thors_replacement) 
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert 0> upper_bound(y) or 0<lower_bound(y), " Interval can't span zero."
