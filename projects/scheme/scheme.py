...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(fn)
        args=rest.map(lambda expression:scheme_eval(expression,env))
...
def do_and_form(expressions, env):
    ...
    current=scheme_eval(first,env)
    if str(current)=='False' or rest is nil:
        return current
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            if second is nil:
                return test
            while second.rest is not nil:
                scheme_eval(second.first,env)
                second=second.rest
            return scheme_eval(second.first, env)
            # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    bindings_temp=bindings
    while bindings_temp is not nil:
        first_pair=bindings_temp.first
        first=first_pair.first
        second=first_pair.rest
        validate_form(second,1,1) 
        names=Pair(first,names)
        values=Pair(scheme_eval(second.first,env),values)
        bindings_temp=bindings_temp.rest
