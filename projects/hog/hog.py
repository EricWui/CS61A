...
def more_boar(player_score, opponent_score):
    ...
    def get(score):
        def get_num(num):
            ...
            return score
        return get_num
    first=get(player_score)(10) < get(opponent_score)(10)
    second=get(player_score)(100) < get(opponent_score)(100)
    if first and second:
        return True
    else:
        return False
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say_highest(score0,score1):
        ...
        if who==0:
            if score0-local_last_score> local_running_high:
                local_running_high=score0-local_last_score
                print(f"Player {who} has reached a new maximum point gain. {local_running_high} point(s)!")               
            local_last_score=score0
            
        elif who==1:
            if score1-local_last_score> local_running_high:
                local_running_high=score1-local_last_score
                print(f"Player {who} has reached a new maximum point gain. {local_running_high} point(s)!") 
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while n<=10:
        ...
        averaged_score=averaged_dice(n,dice)
        if max_average_score <averaged_score:
            max_average_rolls=n
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if more_boar(piggy_points(opponent_score)+score,opponent_score) or piggypoints_strategy(score, opponent_score, cutoff, num_rolls)==0:
        return 0
    else:
        return num_rolls 
    # END PROBLEM 11
