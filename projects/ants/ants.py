...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            if self.is_container() and place.ant.is_container():
                assert place.ant is None, 'Two ants in {0}'.format(place) 
            if self.is_container():
                self.contain_ant(place.ant)
                place.ant=self
            elif place.ant.is_container() and not place.ant.contained_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while index < self.min_range and not nearest_place.is_hive():
            nearest_place=nearest_place.entrance
            index+=1
        while not nearest_place.is_hive() and index < self.max_range and not nearest_place.bees:
            nearest_place=nearest_place.entrance
            index+=1    
        return None if nearest_place.is_hive() else bee_selector(nearest_place.bees)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # self.throw_at(self.nearest_bee(gamestate.beehive))
        if not self.chewing and self.place.bees:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        return self.is_container() and not self.contain_ant
        # END Problem 8
...
# END Problem EC
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.health-=amount
        if self.health<=0 and self.is_true:
            bees_win()
        else:
            super().reduce_health(amount)
        # END Problem EC
